...
- `autoswitch.homeAccount` (or `--home <num|email>`) **pins the live login to one account**. With a home set, auto-switch never rotates off it on its own — not at the threshold, not under `consume-first`, not even at an account-wide (5h/7d) limit (that wall is yours to wait out) — and the things that move it are a dead token (the failover) and, with `autoswitch.model` configured, that model's scoped window reaching the switch threshold (the pinned login's own work is model-bound, so a model-burned home is a mute terminal, not a rest; the escape rides the normal at-limit/proactive triggers). Away from home (after such an escape, or a manual `cswap switch`), it returns the moment the home account reads again **with its model window below the threshold**, ignoring the cooldown; each return is logged as a `switch` with trigger `return-home`, a return blocked by the model window logs a `home-model-burned` no-switch, and a return that cannot land yet (session traffic, a `cswap run` session holding the slot, a network blip) logs a `return-home-wait` no-switch while the normal rotation — at-limit and failover included — keeps judging the account you are on. While the pin is active the pool-shield's burned-host preference stands down: voluntary landings are judged model-aware, so the login is never parked on a host that cannot serve its model. The record's `activeAccountNumber` also follows the real login: a manual `/login` onto another managed account is adopted on the next tick and logged as `adopt-real-login` with both numbers. A disabled or unknown home leaves the pin inert with one `config-warning`. Meant for setups where other accounts are consumed per-terminal through `cswap run` and the default login should stay out of their way: a login that keeps rotating costs such a fleet one seat at all times.
