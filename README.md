...
- `autoswitch.homeAccount` (or `--home <num|email>`) **pins the live login to one account**. With a home set, auto-switch never rotates off it on its own — not at the threshold, not under `consume-first`, not at an account-wide (5h/7d) limit (that wall is yours to wait out), and not when a configured `autoswitch.model` window on the home is burned either (the hold logs a `home-pinned` no-switch that names the burned window; the couriers that use the default login ride their own model ladder): the only things that move it are a dead token (the failover) and your own word — `cswap config set autoswitch.homeAccount` or `cswap disable <home>`. Away from home (after a failover or a manual `cswap switch`), it returns the moment the home account reads again — its model window is not judged — ignoring the cooldown; each return is logged as a `switch` with trigger `return-home`, and a return that cannot land yet (session traffic, a `cswap run` session holding the slot, a network blip) logs a `return-home-wait` no-switch while the normal rotation — at-limit and failover included — keeps judging the account you are on. A live `cswap run` session on the home slot holds the return indefinitely: bring the login home by hand with `cswap switch <home> --even-if-live` (with the override the switch adopts the session's newer generation into the backup instead of refusing). While the pin is active the pool-shield's burned-host preference stands down: voluntary landings are judged model-aware. The record's `activeAccountNumber` also follows the real login: a manual `/login` onto another managed account is adopted on the next tick and logged as `adopt-real-login` with both numbers. A disabled or unknown home leaves the pin inert with one `config-warning`. Meant for setups where other accounts are consumed per-terminal through `cswap run` and the default login should stay out of their way: a login that keeps rotating costs such a fleet one seat at all times.
...
It judges who ran ahead by the profile's seed stamp, never by the stale marker: a profile that rotated past the stored login is left alone (`profile-ahead` — the stored login adopts its generation instead, no grant consumed); a profile holding the consumed generation gets the stored login's `claudeAiOauth` written into its store under Claude Code's own credential locks for that profile, keeping the profile's own MCP logins; an expired stored login is refreshed first, and a rejected grant leaves the profile untouched (`relogin-required`). Live sessions are not a blocker — their PIDs are reported. Claude Code re-reads its store under its own locks before refreshing or forcing re-auth, so a live session is expected to pick the new generation up at its next such re-read — that is verified for the default `~/.claude` store, not separately for session profiles, so treat it as likely, and restart the session (`cswap run N -- --resume`) when you need certainty. The active default login is refused (`active-slot`: its live credential is Claude Code's own store and heals through the collector), as are a profile on an attached setup-token, an API-key slot and a slot without a profile (`token-profile`, `api-key`, `no-profile`); with `--json` a refusal is the usual error envelope plus `error.outcome` and `error.livePids`, exit 1.
...
Every payload carries a `schemaVersion` (currently `1`); on a handled error stdout is `{"schemaVersion":1,"error":{...}}` with a non-zero exit code. `--switch`/`--switch-to` report `{"switched": true|false, "from": …, "to": …, "reason": …}`. `reseed --json` reports `{"reseeded": true|false, "outcome": "reseeded"|"in-sync"|"profile-ahead", "from": "backup"|null, "generation": …, "livePids": [...], "detail": …}`; its refusals carry `error.outcome` (`no-profile`, `active-slot`, `token-profile`, `relogin-required`, …) and `error.livePids` inside the error envelope.
